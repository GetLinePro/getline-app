...
import pro.getline.vpn.design.databinding.DesignPropertiesBinding
import pro.getline.vpn.design.dialog.ModelProgressBarConfigure
import pro.getline.vpn.design.dialog.requestModelTextInput
import pro.getline.vpn.design.dialog.withModelProgressBar
...
class PropertiesDesign(context: Context) : Design<PropertiesDesign.Request>(context) {
    ...
    /**
     * When true, progress dialog uses GetLine vocabulary (no provider names / CMFA jargon).
     * Set for product import auto-commit; Advanced/manual profile edit keeps CMFA copy.
     */
    var useProductFetchVocabulary: Boolean = false
        ...
        try {
            ...
            context.withModelProgressBar {
                configure {
                    ...
                    text = if (useProductFetchVocabulary) {
                        context.getString(R.string.get_line_fetch_loading_config)
                    } else {
    ...
    private fun ModelProgressBarConfigure.applyFrom(status: FetchStatus) {
        if (useProductFetchVocabulary) {
            applyProductVocabulary(status)
            return
        }
        when (status.action) {
    ...
    /**
     * Product path: stable GetLine labels. Raw stage stays in logs.
     * Progress bar still moves for multi-step provider/verify work.
     */
    private fun ModelProgressBarConfigure.applyProductVocabulary(status: FetchStatus) {
        Log.d(GetLineFetchStatusCopy.diagnosticLine(status))
        val stage = GetLineFetchStatusCopy.stageOf(status.action) ?: return
        text = context.getString(GetLineFetchStatusCopy.stringRes(stage))
        when (status.action) {
            FetchStatus.Action.FetchConfiguration -> {
                isIndeterminate = true
            }
